...
def plot_metrics(history, metric_name,lim_y_bottom = 0, lim_y_top = 1.6, file = None):
    plt.plot(history[metric_name], color='navy')
    plt.plot(history[f'val_{metric_name}'], color='red')
...
def model_evaluation(model, X_test, y_test, indices = [1, 2, 3, 4, 5]):
    ...
    mae_e1 = tf.keras.metrics.mean_absolute_error(y_true_e1, predictions_e1)
    ...
    mae_e2 = tf.keras.metrics.mean_absolute_error(y_true_e2, predictions_e2)
    ...
    for i in range(len(predictions)):
        print(f'------------')
        print('real:\t\t', '\t'.join(f'{val:.3f}' for val in y_test.iloc[indices[i]]))
